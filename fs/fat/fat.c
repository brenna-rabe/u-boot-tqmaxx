...
{
	printf("Beginning fat_register_device in fs/fat/fat.c\n");
	...
	printf("Ending fat_register_device in fs/fat/fat.c, returning with call to fast_set_blk_dev\n");
...
{
	printf("Beginning function file_fat_read in /fs/fat/fat.c. File name is %s\n", filename);
	loff_t actread;
	int ret;

	ret =  file_fat_read_at(filename, 0, buffer, maxsize, &actread);
	printf("Ending function file_fat_read in /fs/fat/fat.c. File name is %s\n", filename);
	if (ret)
		return ret;
	else
		return actread;
